...
impl WindowlessRenderer {
    ...
    fn create(
        ...
        unsafe {
            ...
            (sys.lynx_windowless_renderer_bind_activate_system_cursor)(
                raw,
                Some(activate_system_cursor),
            );
            (sys.lynx_windowless_renderer_bind_show_text_input)(raw, Some(show_text_input));
            (sys.lynx_windowless_renderer_bind_update_caret_position)(
                raw,
                Some(update_caret_position),
            );
            (sys.lynx_windowless_renderer_bind_set_cursor_position)(raw, Some(set_cursor_position));
            (sys.lynx_windowless_renderer_bind_set_marked_text_rect)(
                raw,
                Some(set_marked_text_rect),
            );
            (sys.lynx_windowless_renderer_bind_set_editable_transform)(
                raw,
                Some(set_editable_transform),
            );

            match renderer_type {
                sys::kRendererTypeSoftware => {
                    (sys.lynx_windowless_renderer_bind_on_software_present)(
                        raw,
                        Some(on_software_present),
                    );
                }
                sys::kRendererTypeGL | sys::kRendererTypeGLDirect => {
                    (sys.lynx_windowless_renderer_bind_on_gl_make_current)(
                        raw,
                        Some(on_gl_make_current),
                    );
                    (sys.lynx_windowless_renderer_bind_on_gl_clear_current)(
                        raw,
                        Some(on_gl_clear_current),
                    );
                    (sys.lynx_windowless_renderer_bind_on_gl_present)(raw, Some(on_gl_present));
                    (sys.lynx_windowless_renderer_bind_on_gl_create_fbo)(
                        raw,
                        Some(on_gl_create_fbo),
                    );
                    (sys.lynx_windowless_renderer_bind_on_gl_proc_resolver)(
                        raw,
                        Some(on_gl_proc_resolver),
                    );
                }
                sys::kRendererTypeAccelerated => {
