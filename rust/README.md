...
- `lynx-cpp-bridge` provides checked-in C ABI bindings and loads
  `libLynx_clay` with `dlopen`/`dlsym`. Rust-only C ABI shims are provided by
  the dynamic library targets under `platform/dynamic_lib`.
...
`lynx_rust_view_set_frame`. Those shims are intentionally linked into
`libLynx_clay` by `platform/dynamic_lib` so the existing C++ exports with
reference parameters remain unchanged for current C++ embedders.
...
For local Rust SDKs, run `rust/tools/adhoc_sign_macos_sdk.py <sdk-dir>` to
ad-hoc sign `libLynx_clay.dylib` when present.
