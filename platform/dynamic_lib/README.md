...
- `lynx_rust_capi.{h,cc}` exports `lynx_rust_*` wrapper symbols for existing
  C++ CAPI functions that are not directly Rust-FFI friendly.
...
`include/capi/lynx_rust_capi.h`, and `data/icudtl.dat`. macOS packages also
include Lynx resource bundles. Linux packages include optional resource bundles
and `lynx_core.js` files when they are present in the build directory.
